...
significance_level=st.text_input(" Enter significance value α  ")
...
if (st.button("calculate")):
    ...
    st.write(f"""
        T-critical values for a left tail test Tc =  {Left_tailed_test} \n 
        T-critical values for a right tail test
        Tc = + {Right_tailed_test} \n
        T-critical values for a two tail test
        Tc = -{Two_tailed_test}  and Tc = +{abs(Two_tailed_test)}
        """)

    f"""
    #### Whenever you perform a two-tailed test, there will be two critical values. In this case, the T critical values are {Right_tailed_test} and {Left_tailed_test}. Thus, if the test statistic is less than {Left_tailed_test} or greater than {Right_tailed_test}, the results of the test are statistically significant.
    """
